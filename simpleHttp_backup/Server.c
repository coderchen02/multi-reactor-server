...
{
    //1.创建epoll实例 epoll底层实现就是一个红黑树 所以创建实例就是创建红黑树的根节点
    int epfd=epoll_create(1);//参数大于0就行 参数1无实际意义
    if(epfd==-1)
    {
        perror("epoll_create");
        return -1;
...
int acceptClient(int lfd, int epfd)
{
    ...
    struct epoll_event ev;
    ev.data.fd=cfd;
    ev.events=EPOLLIN | EPOLLET;//EPOLLET是用来将epoll设置为边沿模式
...
int sendFile(const char *fileName, int cfd)
{
    ...
    while(1)
    {
        ...
        {
            send(cfd,buf,len,0);
            usleep(10);//这非常重要 休眠10微秒 为了让接收端喘口气
        }
        else if(len==0)
        {
            break;
...
#else
    int size=lseek(fd,0,SEEK_END);//计算打开的文件的文件大小
...
int sendDir(const char *dirName, int cfd)
{
    ...
    for(int i=0;i<num;i++)
    {
        ...
        char subPath[1024]={0};
        sprintf(subPath,"%s/%s",dirName,name);//路径地址和文件名字拼接
        stat(subPath,&st);
        if(S_ISDIR(st.st_mode))
        {   //如果是目录
            //a标签 <a href="">name</a> 用来点击就打开子目录
